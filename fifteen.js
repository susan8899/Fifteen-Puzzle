...
var timer = setInterval(function () {
	...
	function answer(){
		...
		var k = 1;
		for(var i = 0; i <= 3; i++){  //for rows less than 4
			for(var j = 0; j <= 3; j++){ //for col less than 4
	...
	function checkSequence(){
		...
		if(getBox(3, 3).className != 'em_box'){
			return;
		}
		...
		for(var i = 0; i <= 3; i++){ 
			for(var j = 0; j <= 3; j++){
				if(k <= 15 && getBox(i, j).innerHTML != k.toString()){ //if postion(i,j) order does not equal string order
					// Order is incorrect
					return;
				}
				k++;
