...
var timer = setInterval(function () {
	...
	function answer(){
		...
		var k = 1;
		for(var i = 0; i <= 4; i++){  //for rows less than 4
			for(var j = 0; j <= 4; j++){ //for col less than 4
				...
				if(k <= 24){  
					...
					if(j==2 && i==3){
						box.classList.add('pic15');
					}
//for 5x5

					if(j==4 && i==0){
						box.classList.add('pic16');
					}
					if(j==4 && i==1){
						box.classList.add('pic17');
					}
					
					if(j==4 && i==2){
						box.classList.add('pic18');
					}


					if(j==0 && i==4){
						box.classList.add('pic21');
					}
					if(j==1 && i==4){
						box.classList.add('pic22');
					}
					if(j==2 && i==4){
						box.classList.add('pic23');
					}


					if(j==3 && i==3){
						box.classList.add('pic19');
					}if(j==3 && i==4){
						box.classList.add('pic24');
					}
					if(j==4 && i==3){
						box.classList.add('pic24');
					}
	...
	function checkSequence(){
		...
		if(getBox(4, 4).className != 'em_box'){
			return;
		}
		...
		for(var i = 0; i <= 4; i++){ 
			for(var j = 0; j <= 4; j++){
				if(k <= 24 && getBox(i, j).innerHTML != k.toString()){ //if postion(i,j) order does not equal string order
					// Order is incorrect
					return;
				}
				k++;
